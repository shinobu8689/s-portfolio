...
int main(){
	...
	fscanf(fp, "%d", &selectedCount);
	
	#pragma omp parallel for private(i, word, wordInt) shared(bitArray, moduloValue, selectedCount) schedule(dynamic, 500) 
	for (i = 0; i < selectedCount; i++){			// 370106 words in "words_alpha.txt",  466550 in words.txt

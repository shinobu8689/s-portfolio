...
int main(int argc, char *argv[]){
	...
	//if (rank%2 == 0) {srand(32);} else {srand(65);}
	srand(rank);
	...
	if(rank == 0){	// base station
		...
		do {
			...
			// if ( time == 8 && (rank == 3 || rank == 5 || rank == 6) ) {vacancy = 0;} else {vacancy = 5;}
			...
			if (vacancy == 0) { 
				...
				if (!haveAdjVacancy) {	// if adj node is full set type alert
					fprintf(nodeLog, "%d\tN%d\thave no ADJ node w/ vacancy\n", time, rank);
					type = 3;
				}
			...
			if (type == 5) {
				...
				//printf("%d\tN%d\tsend\tTOTAL_MSG\t%d\n", time, rank, totalMsgSent);

...
class Cave:
    ...
    def __init__(self, name: str, material: Material, quantity: float=0) -> None:
        '''
            Initialises a cave, the name, what it stored, and amount of it stored
            Parameters: name (str), material (Material), quantity (float)
            Return:     None
            Complexity: O(1)
        '''
        self.name = name
    ...
    def __str__(self) -> str:
        '''
            ...
            Complexity: O(1)
        '''
        return self.name + ": " + str(self.material) + " - Quantity: " + str(self.quantity)

    @classmethod
    def random_cave(cls, material_list: list[Material]) -> Cave:
        '''
            Generate a random cave based on the provided cave name list
            with the material list given
            Parameters: material_list (list[Material])
            Return:     Cave object
            Complexity: O(1)
        '''
        new_name = CAVE_NAMES[RandomGen.randint(0, len(CAVE_NAMES) - 1)]
        new_material = material_list[RandomGen.randint(0, len(material_list) - 1)]
        new_quantity = RandomGen.randint(1, 10)
        return cls(new_name, new_material, new_quantity)

if __name__ == "__main__":
    print(Cave("Mt Coronet", Material("Coal", 4.5), 3))
    material_list = []
    for n in range(5):
        material_list.append(Material.random_material())
    for n in range(5):
        print(Cave.random_cave(material_list))
